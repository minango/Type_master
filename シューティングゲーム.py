...
for i in range(10):
    x = 20 + i*45
    y = 350
    level_buttons.append(Button((x,y,40,40), colors[i], str(i+1)))
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type==pygame.MOUSEBUTTONDOWN:
            pos = pygame.mouse.get_pos()
            if left_btn.collidepoint(pos): moving_left = True
            if right_btn.collidepoint(pos): moving_right = True
            if shoot_btn.collidepoint(pos): shooting = True
        ...
        if event.type==pygame.MOUSEMOTION:
            pos = pygame.mouse.get_pos()
            moving_left = left_btn.collidepoint(pos)
            moving_right = right_btn.collidepoint(pos)
            shooting = shoot_btn.collidepoint(pos)
    ...
    tracking = cp_levels[cp_level]["tracking"]
    rand = random.choice([-1,1])
    move = direction*tracking*cp_levels[cp_level]["speed"] + rand*(1-tracking)*0.1
    enemy.x += move
    ...
    enemy_ratio=enemy_hp/max_enemy_hp
    enemy_color = RED if enemy_ratio>0.3 else YELLOW
    if not (enemy_ratio<0.2 and pygame.time.get_ticks()%300<150):
        pygame.draw.rect(screen, enemy_color, (20,20,200*enemy_ratio,20))

    pygame.draw.rect(screen, WHITE, (20, HEIGHT-60,200,20),2)
    player_ratio=player_hp/max_player_hp
    player_color = BLUE if player_ratio>0.3 else YELLOW
    if not (player_ratio<0.2 and pygame.time.get_ticks()%300<150):
        pygame.draw.rect(screen, player_color, (20, HEIGHT-60,200*player_ratio,20))
    ...
    # UI
    screen.blit(font_small.render(f"Difficulty: {difficulty_text}", True, WHITE),(WIDTH-180, HEIGHT-30))
    screen.blit(font_small.render(f"Lv:{cp_level+1}", True, WHITE),(WIDTH-120, 20))
